...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    data = items.merge(products, on="product_id", how="left").merge(orders, on="order_id", how="left")
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    orders = read_sql("SELECT order_id, order_purchase_timestamp FROM olist_orders", database)
    ...
    order_purchase_ammount_per_date = filtered_dates.groupby(filtered_dates["order_purchase_timestamp"].dt.date).size().reset_index()
